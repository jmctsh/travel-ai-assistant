...
function getWeatherConfig() {
  ...
  let cityId = '101210101' // 默认为杭州

  if (savedConfig) {
    const config = JSON.parse(savedConfig)
    if (config.weather && config.weather.cityId) {
      cityId = config.weather.cityId
    }
  }

  return {
    // 从环境变量安全地获取API Key
    apiKey: import.meta.env.VITE_QWEATHER_API_KEY || '',
    cityId: cityId
  }
}
...
export async function getWeatherForDate(date, city = 'Hangzhou') {
  try {
    ...
    if (!weatherConfig.apiKey) {
      throw new Error('请在.env.local文件中配置和风天气VITE_QWEATHER_API_KEY')
    }
    
    // 使用和风天气API获取7天天气预报
    const response = await axios.get(`${WEATHER_BASE_URL}/weather/7d`, {
      params: {
        location: weatherConfig.cityId,
